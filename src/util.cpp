...
#else
    const char* pszModule = "bottlecapsplus";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BottleCapsPlus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BottleCapsPlus
    // Mac: ~/Library/Application Support/BottleCapsPlus
    // Unix: ~/.bottlecapsplus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BottleCapsPlus";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bottlecapsplus.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bottlecapsplusd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BottleCapsPlus will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("BottleCapsPlus"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
