...
unsigned int nStakeMinAge = 3 * nOneDay; // 3 days as zero time weight
unsigned int nStakeMaxAge = 20 * nOneDay; // 20 days as full weight
unsigned int nStakeTargetSpacing = 4 * 60; // 4-minute stakes spacing
unsigned int nModifierInterval = 60 * 60; // nOneHour; // time to elapse before new modifier is computed
...
const string strMessageMagic = "BottleCapsPlus Signed Message:\n";
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("bottlecapsplus-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BottleCapsPlus
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // BottleCapsPlus: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // Check timestamp
    if (GetBlockTime() > FutureDrift(GetAdjustedTime()) && nHeight > 250000)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BottleCapsPlus", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
unsigned char pchMessageStart[4] = { 0xc6, 0xeb, 0xef, 0xfd };
