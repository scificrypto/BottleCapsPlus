...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("bottlecapsplus-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #bottlecapsplusTEST2\r");
            Send(hSocket, "WHO #bottlecapsplusTEST2\r");
        } else {
            // randomly join #bottlecapsplus00-#bottlecapsplus05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bottlecapsplus%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bottlecapsplus%02d\r", channel_number).c_str());
        }
